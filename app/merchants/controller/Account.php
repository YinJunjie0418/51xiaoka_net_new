...
class Account extends UserBase
{
      public function initialize()
    {
        ...
        if(in_array($this->user['userReal']['retype'],[1,2]) && $action!='uploadImage' && $action!='enterprise'){
            $this->redirect(url('merchants/Member/realname'));
        }
        if($this->user['userReal']['retype']==3)$this->redirect(url('merchants/Member/realname'));
    }

	public function index(){
	    if($this->user['userReal']['clas']==2)$this->redirect(url('merchants/Account/enterprise'));
        if($this->request->isAjax()){
            ...
            if(request()->isMobile()){
                ...
                $ress=(new pcticket())->pcLoginUrl($data['username'],$data['idcard'],$order,"",$data['idjust'],$data['idback']);
                if($ress['code']!=1){
                ...
                $udata=['uid'=>session('user_auth.user_id'),'username'=>$data['username'],'idcard'=>$data['idcard'],'imga'=>$data['idjust'],'imgb'=>$data['idback'],'time'=>time()];
                $enstr=Order::enCardno(json_encode($udata));
                cache((string)session('user_auth.user_id'),$enstr,600);
                $url=$http.$_SERVER['HTTP_HOST'].url('merchants/wxface/index',['id'=>session('user_auth.user_id')]);
    ...
    public function uploadImage()
    {
        try{
			...
			foreach($files as $k=>$v){
				validate([$k=>'fileSize:1048576|fileExt:xls,xlsx'])->check($files);
			}
