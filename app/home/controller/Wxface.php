    ...
    public function index(){
        ...
        if($id){
            if(cache($id)){
                ...
                if(isset($data['username']) && isset($data['idcard']) && isset($data['uid'])){
                    $http="http://";
                    if(is_https())$http="https://";
                    $order=generate_password(16);
                    cache($order.'bbs',$data,1200);
                    $ress=(new pcticket())->pcLoginUrl($data['username'],$data['idcard'],$order,$http.$_SERVER['HTTP_HOST'].url('home/Wxface/facequery'));
                }else{
